...
def upgrade() -> None:
    ...
    if inspector.has_table("number_bomb_games"):
    ...
    if inspector.has_table("number_bomb_members"):
        with op.batch_alter_table("number_bomb_members") as batch:
            ...
            batch.add_column(
                sa.Column("retired_at_round", sa.Integer(), nullable=True)
            )
    if inspector.has_table("number_bomb_round_players"):
