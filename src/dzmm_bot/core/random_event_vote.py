...
def tiered_pool(
    ...
    *,
    ...
    """把场景名按优先级排成一列：越靠前越该被选中。

    三档依次是「没演过且没排过」「演过但没排过」「今天已排过」——与既有随机
    逻辑一致；区别只在于既有逻辑是**只取命中的那一档**，而这里把三档拼起来，
    让调用方能在第一档不够时跨档补齐到想要的候选数。
    """
